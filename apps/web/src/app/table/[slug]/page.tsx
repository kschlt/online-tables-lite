...
export default function TablePage() {
  ...
  useEffect(() => {
    ...
    const fetchTable = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/table/${slug}`, {
          headers: {
            't': token
          }
        })
    ...
    <main className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-full mx-auto">
        ...
        <div className="bg-white rounded-lg shadow overflow-x-auto">
          <div className="grid gap-0 min-w-full" style={{ gridTemplateColumns: `repeat(${tableData.cols}, 1fr)` }}>
          ...
          {Array.from({ length: tableData.rows }).map((_, rowIndex) => (
            <div key={rowIndex} className="grid gap-0" style={{ gridTemplateColumns: `repeat(${tableData.cols}, 1fr)` }}>
              {tableData.columns.map((column) => (
                <div
                  key={column.idx}
