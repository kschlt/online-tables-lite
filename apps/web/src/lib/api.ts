...
import type { TableData, CreateTableRequest, CreateTableResponse, CellBatchUpdateRequest, TableConfigRequest, TableConfigResponse } from '@/types';
...
export const api = {
  ...
  async updateCells(slug: string, token: string, request: CellBatchUpdateRequest): Promise<{ success: boolean; updated_cells: number }> {
    return apiRequest<{ success: boolean; updated_cells: number }>(`${API_ENDPOINTS.TABLES}/${slug}/cells`, {
      method: 'POST',
      token,
      body: JSON.stringify(request),
    });
  ...
  async updateTableConfig(slug: string, token: string, request: TableConfigRequest): Promise<TableConfigResponse> {
